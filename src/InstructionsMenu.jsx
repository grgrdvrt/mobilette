        ...
        <div class="instructionsMenu">
          <Show when={selectedModule() !== null}>
            ...
            <For each={Object.keys(instructionsDefinitions[selectedModule()])}>
              {(instructionName) => {
                  return(
                      <li><button onClick={() => setCommand(selectedModule(), instructionName)}>{instructionName}</button></li>
                  );
              }}
            </For>
            </ul>
          ...
          <For each={Object.keys(instructionsDefinitions)}>
            {(moduleName) => {
                return(
                    <li><button onClick={() => setSelectedModule(moduleName)}>{moduleName}</button></li>
                );
            }}
          </For>
          </ul>
